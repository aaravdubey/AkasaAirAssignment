      ...
      <nav className={isHome ? "bg-transparent" : "bg-white shadow"}>
        <div className={`max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4 ${isHome ? "py-5" : "py-3"}`}>
          ...
          <div className="flex gap-5">
            ...
            <button className={`w-full md:flex md:w-auto ${isHome ? "hover:bg-slate-200" : "hover:bg-slate-100"} justify-center rounded-full cursor-pointer pl-4 pr-2 py-1 transition-all duration-150`} onClick={() => setSidebarVisible(true)}>
              <div className="w-full inline-flex justify-between items-center gap-1.5 mr-3 text-sm text-logo-green ">
                ...
                <div>
                  <a href="#" rel="author" className="text-sm font-medium">{localStorage.getItem('name')}</a>
                </div>
              </div>
            </button>
          </div>
        ...
        <div className='w-[22.5%] bg-white py-3'>
          <div className='mt-2 border-b border-gray-200 px-2'>
            <div className='flex w-full justify-start items-center'>
              ...
              <div className='py-4'>
                <p className='text-xl font-semibold'>Hello, {localStorage.getItem("name")}</p>
